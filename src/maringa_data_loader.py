...
def load_maringa_data() -> pd.DataFrame:
    ...
    df["indice_infraestrutura_urbana"] = (
        df["inst_saude_norm"] + df["inst_educacao_norm"] + df["transporte_publico_norm"]
    ) / 3

    df["indice_qualidade_urbana"] = (
        df["espacos_publicos_norm"] + df["inst_culturais_norm"] +
        df["inst_esportivas_norm"] + df["indice_infraestrutura_urbana"]
    ) / 4

    df["indice_ocupacao_solo"] = (
        df["densidade_construida"] + df["intensidade_ocupacao_norm"]
    ) / 2

    df["indice_ociosidade_urbana"] = df["lotes_vagos_norm"]

    df["indice_adensamento_inteligente"] = (
        df["densidade_habitacional_norm"] * df["indice_ocupacao_solo"]
    )
